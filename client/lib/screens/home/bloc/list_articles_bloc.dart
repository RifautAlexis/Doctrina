...
import 'package:client/blocs/bloc.dart';
...
class ListArticlesBloc extends Bloc<ListArticlesEvent, ListArticlesState>{
  ...
  ListArticlesBloc({@required this.articleService}) : super(ListArticlesInitial());

  @override
  void mapEventToState(ListArticlesEvent event) async {
    if (event is ArticlesFetched) {
      try {
        final articles = await _fetchArticles();
        updateState.add(ListArticlesSuccess(articles: articles));
        return;
      } catch (_) {
        updateState.add(ListArticlesFailure());
      }
    }
  }
